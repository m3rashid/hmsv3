import { Form, Button, AutoComplete, message, Typography, Row, Col, Space, Card } from 'antd';
...
const CreateAppointmentForm = () => {
  ...
  useEffect(() => {
    socket.on('new-appointment-created', (data) => {
      setLoading(false);
      message.success(`Appointment for ${data?.patient?.name} created successfully!`);
    });
            ...
            label: (
              <Col direction="vertical" size={'small'} style={{ fontSize: 12 }}>
                <Row>
                  ...
                  {patient.userId && (
                    <Typography.Text type="success">{`(${patient.userId})`}</Typography.Text>
                  )}
                  {patient.department && (
                    <Typography.Text type="danger">{`${patient.department}`}</Typography.Text>
                  )}
                  {patient.contact && (
      ...
      <Row>
        <Col span={12} style={{ paddingRight: 15 }}>
          ...
          >
            ...
            >
              <DoctorSelector
                onChange={(value) => setFormSelected({ ...FormSelected, doctor: value })}
              />
            </Form.Item>
            <Form.Item
              label="Date"
            ...
            >
              <Fragment>
                <AutoComplete
                  ...
                  allowClear
                  onSelect={(value) => {
                    const patientData = patients.data.find((patient) => patient.value === value);
                  ...
                  onClear={() => setFormSelected({ ...FormSelected, patient: null })}
                  onChange={(value) => {
                    const patientData = patients.data.find((patient) => patient.value === value);
                    if (!patientData) {
          ...
          <Row>
            ...
            <Col span={20} style={{ marginBottom: 10 }}>
              <Typography.Text level={4} type="danger">
                {form.getFieldValue('datetime')?.format('YYYY-MM-DD HH:mm A') || 'N/A'}
              </Typography.Text>
            </Col>
          </Row>
            ...
            <Card title="Patient Details">
              {FormSelected.patient ? (
                <Space direction="vertical">
                  ...
                  <Typography.Text>{FormSelected.patient?.data?.name}</Typography.Text>
                  <Typography.Text>Contact : {FormSelected.patient?.data?.contact}</Typography.Text>
                  <Typography.Text disabled>{FormSelected.patient?.data?.email}</Typography.Text>
                </Space>
              ) : (
