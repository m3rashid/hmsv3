...
import { Divider, Radio, Table, Form, Input } from 'antd';
...
const MedicineTable = ({ medicines = [], selectedMedicines, setSelectedMedicines }) => {
  ...
  useEffect(() => {
    setDispensingMedicines(medicines.map(med => {
      const requiredQuantity =  Math.ceil(med.duration * DOSAGE_MAP[med.dosage])
      return {
        ...med,
        requiredQuantity,
        amountDispensed: 0,
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text, record) => {
        return <Input onChange={(e)=>handlerDispensedAmount(record.id, e.target.value)} type='number' min={0} max={Math.min(record.requiredQuantity, record.stock)} value={record.amountDispensed} />
      }
    }
  ];
