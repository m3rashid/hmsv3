...
const useGetUserDetail = ({ userType, userRole }) => {
  ...
  const getSinglePatientDetail = async (id) => {
    ...
    const filteredData = Object.entries(res.data).reduce((acc, [key, value]) => {
      if (!value || value === 'null' || value === 'undefined' || value === '') return acc;
  ...
  const getUsers = async () => {
    try {
      ...
      const users = userRole !== 'PATIENT' ? formatForTable(res.data.users) : res.data.users;
  ...
  const RefreshUserButton = () => {
    return (
      <Button
        onClick={() => getUsers().then(message.success('Users fetched successfully')).catch()}
      >
        Refresh Users
      </Button>
    );
