...
export const handleDataMigration = async (req: Request, res: Response) => {
	...
	const supportedSheets = ['Employee', 'Pensioner', 'Family Pensioner', 'Dependent'];

	const sheets = wb.SheetNames.filter((sheet) => supportedSheets.includes(sheet));

	if (sheets.length !== supportedSheets.length) {
		throw new Error('Sheets did not match the expected sheetNames');
	}

	const [employeeSheet, pensionerSheet, familyPensionerSheet, dependentSheet] = supportedSheets.map(
		(s) =>
	...
	const p3 = handleMigrationService('FAMILY_PENSIONER', familyPensionerSheet, 'EMP ID');
